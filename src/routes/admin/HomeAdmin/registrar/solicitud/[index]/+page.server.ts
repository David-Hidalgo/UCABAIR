...
import type {Compra, Lote_materia_prima, Tipo_materia_prima, Proveedor2} from '$lib/server/db/schema';
export const load: PageServerLoad=async({ params}) =>{
    ...
    if (Number.isInteger(n)) {
        ...
        const datos_solicitud= await dbPostgre<Lote_materia_prima[]>`select * from lote_materia_prima where fk_configuracion_pieza=${n}`;
        const materia_prima = await dbPostgre<Tipo_materia_prima[]>`select * from tipo_materia_prima where codigo_tmp=${n}`; 
        const proveedor= await dbPostgre<Proveedor2[]>`select DISTINCT pro.codigo_com as codigo_com, pro.denominacion_persona_jur as denominacion_comercial ,pro.razon_social_jur as razon_social,
