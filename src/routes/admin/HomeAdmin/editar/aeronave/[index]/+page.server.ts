...
export const load: PageServerLoad = async ({ params }) => {
	...
	if (Number.isInteger(n)) {
		...
		const [caracteristicas_asignadas] = await dbPostgre<
			Caracteristica_modelo[]
		>`select * from caracteristica_modelo where fk_modelo_avion=${n}`;
		const piezas = await dbPostgre<
			Tipo_pieza[]
		>`select * from configuracion_avion ca where ca.fk_modelo_avion=${n}`;
