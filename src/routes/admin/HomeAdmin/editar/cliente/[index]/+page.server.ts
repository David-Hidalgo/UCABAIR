...
export const load: PageServerLoad = async ({ params }) => {
	...
	if (Number.isInteger(n)) {
		const [resultado] = await dbPostgre<Persona[]>`select * from persona where codigo_com=${n}`;
		const telefonos = await dbPostgre<Telefono[]>`select * from telefono where fk_persona=${n}`;
		const correos = await dbPostgre<
			Correo_electronico[]
		>`select * from correo_electronico where fk_persona=${n}`;
		const fk_usuario = await dbPostgre<Persona[]>`select p.fk_usuario from persona p, usuario u 
