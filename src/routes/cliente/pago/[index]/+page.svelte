<script lang="ts">
	...
	import type { Modelo_avion,Persona,Telefono,Correo_electronico,Modo_pago } from '$lib/server/db/schema';
...
<div class="InformacionPago">
	...
	<div class="DatosCliente">
		{#if persona.tipo_persona_com === 'natural'}
			...
			<p>{persona.primer_nombre_nat} {persona.segundo_nombre_nat} 
			{persona.primer_apellido_nat} {persona.segundo_apellido_nat}</p>
		{:else}
			<h3>RIF:</h3>
			<p>{persona.rif_jur}</p>
			<h3>Denominacion:</h3>
...
{#if $selectedComponent === InformacionPagoTarjeta}
	<InformacionPagoTarjeta bind:modo_pago />
{:else if $selectedComponent === InformacionPagoTransferencia}
	<InformacionPagoTransferencia bind:modo_pago/>
{:else if $selectedComponent === InformacionPagoCheque}
	<InformacionPagoCheque bind:modo_pago/>
{/if}
