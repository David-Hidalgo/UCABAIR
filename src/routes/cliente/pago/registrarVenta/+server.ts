...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (datosVenta.moneda.nombre_mon === 'Bolívar') {
		console.log('entré aca');
		[respuesta] =
			await dbPostgre`call insertar_pago_venta(${precioAvion},${impuesto},${total},${datosVenta.modo_pago as number},${datosVenta.moneda.codigo_mon as number},${datosVenta.codigo_com as number},${cantidad})`;
		...
		[respuesta] =
			await dbPostgre`call insertar_pago_venta(${precioAvion},${impuesto},${total},${datosVenta.modo_pago},${datosVenta.moneda.codigo_mon},${datosVenta.codigo_com},${cantidad})`;
		return json({ respuesta }, { status: 201 });
	}
};
