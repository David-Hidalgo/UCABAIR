...
export const POST: RequestHandler = async ({ request }) => {
	...
	if (datosVenta.moneda.nombre_mon === 'Bolívar') {
		console.log('entré aca');
		[respuesta] =
			await dbPostgre`call insertar_pago_venta(${precioAvion},${impuesto},${total},${datosVenta.modo_pago as number},${datosVenta.moneda.codigo_mon as number},${datosVenta.codigo_com as number},${cantidad},${datosVenta.codigo_ma})`;
		...
		total =
			datosVenta.precioAvion / datosVenta.moneda.tasa_cambio_mon +
			(datosVenta.precioAvion / datosVenta.moneda.tasa_cambio_mon) * 0.03;
		[respuesta] =
			await dbPostgre`call insertar_pago_venta(${precioAvion},${impuesto},${total},${datosVenta.modo_pago},${datosVenta.moneda.codigo_mon},${datosVenta.codigo_com},${cantidad},${datosVenta.codigo_ma})`;
		return json({ respuesta }, { status: 201 });
	}
};
